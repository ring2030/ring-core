...
// senderName は実機と同じ日本語名（清子・太郎・花子）を使用。
// Firestore batches are capped at 500 ops; we chunk commits automatically.
//
// Usage:
//   GET /api/dev/seed-demo?key=SEED_DEMO_SECRET          → seed
//   GET /api/dev/seed-demo?key=SEED_DEMO_SECRET&action=clean  → clean
...
// [reason, transcript（患者の発話）, staffNote, aiSummary, priority]
...
interface SeededCall { scenario: Scenario; hour: number; minute: number }
 ...
 * - 会話率 ~75%。
 */
function planDay(persona: Persona, dayOffset: number, idx: number): SeededCall[] {
  const rand = mulberry32(dayOffset * 7919 + idx * 104729 + 13);
  ...
  for (let i = 0; i < target; i++) {
    ...
    for (const b of buckets) { acc += b[2]; if (r <= acc) { bucket = b; break; } }
...
export async function GET(req: NextRequest) {
  ...
  const hospitalId = process.env["DEMO_HOSPITAL_ID"]?.trim() || DEFAULT_HOSPITAL_ID;
  const collectionName = getCallsCollectionNameForHospital(hospitalId);
  const action = req.nextUrl.searchParams.get("action") ?? "seed";

  // ─── CLEAN ──────────────────────────────────────────────
  if (action === "clean") {
    const snap = await db.collection(collectionName).where("seedTag", "==", SEED_TAG).get();
    if (snap.empty) return NextResponse.json({ ok: true, removed: 0 });
  ...
  for (let day = -DAY_RANGE_BACK; day <= DAY_RANGE_FORWARD; day++) {
    PERSONAS.forEach((persona, pi) => {
      for (const seeded of planDay(persona, day, pi)) {
        ...
        perResident[persona.senderName] = (perResident[persona.senderName] ?? 0) + 1;
  ...
  return NextResponse.json({
    ...
    days: DAY_RANGE_BACK + 1,
    message: `✅ ${pending.length} 件を書き込みました (${DAY_RANGE_BACK + 1} 日分 × 3 名) — ${collectionName}`,
  });
}
